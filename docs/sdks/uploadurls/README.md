...
async function run() {
  const result = await sdkNodePlatform.uploadUrls.generate();
...
import { uploadUrlsGenerate } from "@amp-labs/sdk-node-platform/funcs/uploadUrlsGenerate.js";
...
async function run() {
  const res = await uploadUrlsGenerate(sdkNodePlatform);
